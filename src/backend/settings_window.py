...
with evaluateJavaScript(window.pysarApply(...)), so the user keeps their place
...
function send(action, value){
  ...
  window.webkit.messageHandlers.pysarBridge.postMessage(JSON.stringify(msg));
}
...
class SettingsWindow:
    ...
    def refresh(self, notice: str | None = None) -> None:
        ...
        if notice:
            state["notice"] = notice
        js = f"window.pysarApply({_encode(state)});"
        with contextlib.suppress(Exception):
    ...
    def _build(self) -> None:
        ...
        config.userContentController().addScriptMessageHandler_name_(self._bridge, "pysarBridge")
