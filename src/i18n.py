...
_UK = {
    ...
    "keep.unit": "записів",
    "dataset.label": "Збирати датасет голосу",
    "dataset.help": "Архівувати кожну диктовку в повній смузі разом із текстом — щоб потім натренувати власний голос",
    "dataset.folder": "Папка датасету",
    "dataset.clips": "кліпів",
    "dataset.hours": "год",
    ...
    "ft.intro": "Обери відео- чи аудіофайли (або цілу теку) — вони декодуються й розпізнаються локально у фоні, по черзі. Результат — Markdown-транскрипти з таймкодами в теці транскриптів.",
    "ft.lang.label": "Мова транскрипції",
    "ft.lang.help": "Мова, якою говорять у файлі",
    "ft.pick.label": "Медіафайли",
    "ft.pick.help": "Файли або тека (сканується рекурсивно); можна додавати й на ходу, поки черга працює",
    "ft.pick.btn": "Обрати файли…",
    "ft.pick.add": "Додати в чергу…",
    "ft.out.label": "Тека збережень",
    "ft.out.help": "Куди складати готові транскрипти",
    "ft.out.choose": "Змінити…",
    "ft.out.reset": "За замовчуванням",
    ...
    "thermal.gentle": "Бережно — пауза на 88°",
    "thermal.normal": "Звичайно — пауза на 95°",
    "thermal.hot": "До упору — пауза на 101°",
    "ft.scanning": "Перевіряю файли…",
    "ft.pausing": "Завершую поточний фрагмент…",
    "ft.paused": "Пауза",
    "ft.cancelled": "Скасовано",
    "ft.qdone": "Готово",
    "ft.pause": "Пауза",
    "ft.resume": "Продовжити",
    "ft.cancelAll": "Скасувати все",
    "ft.item.pending": "у черзі",
    "ft.item.done": "готово",
    "ft.item.diar": "розділяю спікерів…",
    "diar.speakers.label": "Скільки голосів",
    "diar.mic.label": "Шукати голоси і в мікрофоні",
    "diar.mic.help": "Вимкнено: усе з твого мікрофона — це ти. Увімкни, якщо в один мікрофон говорили двоє (одна кімната, один ноут).",
    "diar.speakers.help": "Якщо памʼятаєш, скільки людей говорило — назви число, і розділення його триматиметься. «Авто» — рушій рахує сам.",
    "diar.speakers.auto": "Авто",
    "ft.item.skipped": "пропущено",
    "ft.item.cancelled": "скасовано",
    "ft.item.remove": "Прибрати з черги",
    "ft.reveal": "Показати у Finder",
    "ft.noFfmpeg": "ffmpeg не знайдено — встанови: brew install ffmpeg",
    "ft.notif.done": "Транскрипцію файлів завершено",
    "ft.notif.cancelled": "Транскрипцію файлів скасовано",
    ...
    "pbank.pick.ph": "Збережені промти…",
    "pbank.name.ph": "назва пресета…",
    "pbank.save": "Зберегти",
    "pbank.delete": "Видалити вибраний пресет",
    "ft.prompt.example": "Приклад: «Вебінар про Claude Code: MCP, субагенти, скіли; спікер — Олена». Просто перелічи теми, імена й терміни, які звучать у файлі.",
    "ft.needPrompt": "Впиши контекст-підказку вище або перемкни джерело на «Авто» — інакше вибір файлу заблоковано.",
    "mscreen.title": "Запис зустрічей",
    "mscreen.intro": "Системний звук (і мікрофон) розпізнаються локально й течуть у вікно транскрипту. Старт — у меню в треї; зупинити можна там само або кнопкою «Зупинити» на самому вікні транскрипту. Тут лише налаштування.",
    ...
    "meeting.source.help": "Наскільки глибоко розкладати мовців: без розмітки → окремо мікрофон і система → ще й за голосом усередині кожного каналу",
    "meeting.source.off": "Без розмітки",
    "meeting.source.fast": "Швидко (за гучністю)",
    "meeting.source.smart": "Розумно (точно)",
    "meeting.source.split": "Розділяти й за голосом (після запису)",
    "meeting.diar.label": "Розділяти спікерів після запису",
    "meeting.diar.help": "Кількох людей в одному каналі розділяє за голосом — один прохід після Стоп, у ОКРЕМИЙ файл поруч із транскриптом",
    "ft.diar.help": "Додає другий файл із текстом, розкладеним по голосах; сам транскрипт лишається без змін",
    "ft.diar.off": "Вимкнено",
    "ft.diar.split": "Розділити спікерів після транскрипту",
    "diar.install": "Завантажити моделі ({mb} МБ)",
    "diar.working": "Завантажую…",
    "diar.starting": "Починаю…",
    "diar.ready": "✅ Розпізнавання голосів готове",
    "diar.need": "Потрібні моделі розпізнавання голосів — {mb} МБ, качаються один раз",
    "diar.readyTitle": "Розділення спікерів готове",
    "diar.readyMsg": "Наступний запис буде розкладено по голосах",
    ...
    "st.datasetOn": "🎙 Збираю датасет голосу",
    "st.datasetOff": "Датасет голосу: вимкнено",
    ...
    "hud.meetingStopping": "Зупиняю, дочищаю хвіст…",
    "hud.meetingRecovering": "Відновлюю захоплення 🔁",
    "hud.captureLost": "Захоплення втрачено 🎧",
    # Transcribe everything (system audio + mic → transcript window).
    # "Транскрибувати все" читалося як пакетна дія над файлами ("транскрибуй усе,
    ...
    "notif.diarDoneTitle": "Спікерів розділено",
    "notif.diarDoneMsg": "Файл: {path}",
    "notif.diarFailedTitle": "Не вдалося розділити спікерів",
    "notif.diarNotReadyTitle": "Розділення спікерів не готове",
    ...
    # Capture that died mid-session, and the raw recovery buffer (24.08.2026).
    "notif.captureRestartTitle": "Звук пропав під час запису",
    "notif.captureRestartMsg": "Захоплення впало і перезапускається. Перевір мікрофон — шматок розмови міг не записатись.",
    "transcript.captureRestart": "⚠️ Тут захоплення звуку впало і було перезапущено — можлива прогалина в записі.",
    "notif.micPinFailedTitle": "Пишу НЕ тим мікрофоном",
    "notif.micPinFailedMsg": "Не вдалося прив'язати «{name}» до захоплення — звук піде з мікрофона за замовчуванням. Вибір пристрою потребує macOS 15+.",
    "notif.noAudioTitle": "‼️ Звуку не було",
    "notif.noAudioMsg": "Запис ішов, але жодного звуку не надійшло. Перевір мікрофон і дозвіл «Запис екрана».",
    "notif.silentGapTitle": "‼️ У записі є провал",
    "notif.silentGapMsg": "{minutes} хв поспіль — повна тиша. Перевір запис одразу.",
    "notif.rawAudioTitle": "💾 Аудіо збережено",
    "notif.rawAudioMsg": "{minutes} хв сирого запису: {path}",
    "meeting.keep.label": "Тримати записів зустрічей",
    "meeting.keep.help": "Аудіо зустрічей у теці meetings; старіші сесії видаляються автоматично",
    "meeting.keep.unit": "зустрічей",
    "meeting.keep.all": "Усі (не видаляти)",
    "meeting.audioFolder.label": "Тека аудіозаписів",
    "st.meetingKeep": "Тримаю останні {n} записів зустрічей",
    "st.meetingKeepAll": "Записи зустрічей не видаляються",
    ...
    "notif.diarReadyTitle": "Розділення спікерів готове 👥",
    "notif.diarReadyMsg": "Моделі докачано — записи зустрічей ділитимуться на мовців.",
    "notif.diarFailTitle": "Розділення спікерів не завелося",
...
_EN = {
    ...
    "dataset.label": "Collect voice dataset",
    "dataset.help": "Archive every dictation full-band with its transcript, to train your own voice",
    ...
    "ft.intro": "Pick video or audio files (or a whole folder) — they are decoded and recognized locally in the background, one by one. The result is Markdown transcripts with timecodes in the transcripts folder.",
    "ft.lang.label": "Transcription language",
    "ft.lang.help": "The language spoken in the file",
    "ft.pick.label": "Media files",
    "ft.pick.help": "Files or a folder (scanned recursively) — more can be added while the queue runs",
    "ft.pick.btn": "Choose files…",
    "ft.pick.add": "Add to queue…",
    "ft.out.label": "Storage folder",
    "ft.out.help": "Where finished transcripts are written",
    "ft.out.choose": "Change…",
    "ft.out.reset": "Use default",
    ...
    "thermal.gentle": "Gentle — pause at 88°",
    "thermal.normal": "Normal — pause at 95°",
    "thermal.hot": "To the limit — pause at 101°",
    "ft.scanning": "Checking files…",
    "ft.pausing": "Finishing the current fragment…",
    "ft.paused": "Paused",
    "ft.cancelled": "Cancelled",
    "ft.qdone": "Done",
    "ft.pause": "Pause",
    "ft.resume": "Resume",
    "ft.cancelAll": "Cancel all",
    "ft.item.pending": "queued",
    "ft.item.done": "done",
    "ft.item.diar": "splitting speakers…",
    "diar.speakers.label": "How many voices",
    "diar.mic.label": "Look for voices in the mic too",
    "diar.mic.help": "Off: everything from your mic is you. Turn on when two people spoke into the same mic (one room, one laptop).",
    "diar.speakers.help": 'If you remember how many people spoke, say the number and the split will hold to it. "Auto" lets the engine count.',
    "diar.speakers.auto": "Auto",
    "ft.item.skipped": "skipped",
    "ft.item.cancelled": "cancelled",
    "ft.item.remove": "Remove from queue",
    "ft.reveal": "Show in Finder",
    "ft.noFfmpeg": "ffmpeg not found — install it: brew install ffmpeg",
    "ft.notif.done": "File transcription finished",
    "ft.notif.cancelled": "File transcription cancelled",
    ...
    "pbank.pick.ph": "Saved prompts…",
    "pbank.name.ph": "preset name…",
    "pbank.save": "Save",
    "pbank.delete": "Delete selected preset",
    "ft.prompt.example": "Example: “A webinar about Claude Code: MCP, subagents, skills; speaker — Elena”. Just list the topics, names and terms heard in the file.",
    "ft.needPrompt": "Type a context hint above or switch the source to “Auto” — file picking is blocked until then.",
    "mscreen.title": "Meeting recording",
    "mscreen.intro": "System audio (and the mic) are recognized locally and stream into the transcript window. Start from the menu-bar item; stop there or with the Stop button on the transcript window itself. This screen is settings only.",
    ...
    "meeting.source.help": "How deeply to separate speakers: no labels → mic and system apart → and by voice inside each channel",
    "meeting.source.off": "Off (no labels)",
    "meeting.source.fast": "Fast (by loudness)",
    "meeting.source.smart": "Smart (accurate)",
    "meeting.source.split": "Also split by voice (after recording)",
    "meeting.diar.label": "Split speakers after recording",
    "meeting.diar.help": "Tells several people in one channel apart by voice — one pass after Stop, into a SEPARATE file next to the transcript",
    "ft.diar.help": "Adds a second file with the text split by voice; the transcript itself is unchanged",
    "ft.diar.off": "Off",
    "ft.diar.split": "Split speakers after transcription",
    "diar.install": "Download models ({mb} MB)",
    "diar.working": "Downloading…",
    "diar.starting": "Starting…",
    "diar.ready": "✅ Voice recognition is ready",
    "diar.need": "Needs the voice-recognition models — {mb} MB, downloaded once",
    "diar.readyTitle": "Speaker splitting is ready",
    "diar.readyMsg": "The next recording will be split by voice",
    ...
    "st.datasetOn": "🎙 Collecting voice dataset",
    "st.datasetOff": "Voice dataset: off",
    ...
    "hud.meetingStopping": "Stopping, flushing the tail…",
    "hud.meetingRecovering": "Recovering capture 🔁",
    "hud.captureLost": "Capture lost 🎧",
    # Transcribe everything (system audio + mic → transcript window).
    ...
    "notif.diarDoneTitle": "Speakers split",
    "notif.diarDoneMsg": "File: {path}",
    "notif.diarFailedTitle": "Could not split speakers",
    "notif.diarNotReadyTitle": "Speaker splitting is not ready",
    ...
    # Capture that died mid-session, and the raw recovery buffer (24.08.2026).
    "notif.captureRestartTitle": "Audio dropped mid-recording",
    "notif.captureRestartMsg": "Capture died and is restarting. Check the microphone — a piece of the conversation may be missing.",
    "transcript.captureRestart": "⚠️ Audio capture died here and was restarted — the recording may have a gap.",
    "notif.micPinFailedTitle": "Recording from the wrong mic",
    "notif.micPinFailedMsg": "Couldn't pin “{name}” to the capture — the system default input will be used instead. Picking the device needs macOS 15+.",
    "notif.noAudioTitle": "‼️ There was no sound",
    "notif.noAudioMsg": "The recording ran, but not a single audio buffer arrived. Check the microphone and the Screen Recording permission.",
    "notif.silentGapTitle": "‼️ There is a gap in the recording",
    "notif.silentGapMsg": "{minutes} min in a row are pure silence. Check the recording now.",
    "notif.rawAudioTitle": "💾 Audio saved",
    "notif.rawAudioMsg": "{minutes} min of raw audio: {path}",
    "meeting.keep.label": "Keep meeting recordings",
    "meeting.keep.help": "Meeting audio in the meetings folder; older sessions are deleted automatically",
    "meeting.keep.unit": "meetings",
    "meeting.keep.all": "All (never delete)",
    "meeting.audioFolder.label": "Audio recordings folder",
    "st.meetingKeep": "Keeping last {n} meeting recordings",
    "st.meetingKeepAll": "Meeting recordings are never deleted",
    # Enhance — post-dictation LLM styling via Ollama.
    ...
    "notif.diarReadyTitle": "Speaker separation ready 👥",
    "notif.diarReadyMsg": "Models downloaded — meeting recordings will be split by speaker.",
    "notif.diarFailTitle": "Speaker separation did not start",
